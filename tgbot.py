...
#     if call.data == 'yes':
#         markup_inline = types.InlineKeyboardMarkup()
#         item_id1 = types.InlineKeyboardButton(text='Песочница', callback_data='Песочница')
#         item_id2 = types.InlineKeyboardButton(text='Стелс', callback_data='Стелс')

#         markup_inline.add(item_id1, item_id2)
#         bot.send_message(call.message.chat.id, 'Нажмите на одну из кнопок', 
#             reply_markup=markup_inline)
#     elif call.data == 'no':
#         bot.send_message(call.message.chat.id, 'Ок, может быть в другой раз')
            

# @bot.callback_query_handler(func = lambda call: True)
# def your_game(call):
#     if call.data  == 'Песочница':
#         bot.send_message(call.message.chat.id, 'Майнкрафт')
#     elif call.data  == 'Стелс':
#         bot.send_message(call.message.chat.id, 'Ассасин')
bot.polling()
